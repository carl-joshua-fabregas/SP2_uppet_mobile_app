...
export default function ViewMyApplicationCard({ adoptionApp }) {
  ...
  const profilePhoto =
    pet?.photos?.find((p) => p.isProfile)?.url || pet?.photos?.[0]?.url;
  ...
  if (!pet) return null; // Safety check in case pet data is missing

  return (
    <TouchableOpacity
      style={styles.cardContainer}
      onPress={onViewPetPress}
      activeOpacity={0.8}
    >
      {/* Pet Thumbnail */}
      <Image
...
const styles = StyleSheet.create({
  ...
  badgeApproved: { backgroundColor: Themes.COLORS.badge || "#D1FAE5" },
  badgeRejected: { backgroundColor: "#FEE2E2" },
  badgeTextPending: { color: "#D97706" },
  badgeTextApproved: { color: Themes.COLORS.badgeText || "#065F46" },
