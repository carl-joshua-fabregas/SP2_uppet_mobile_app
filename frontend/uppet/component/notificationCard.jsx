...
export default function NotificationCard(props) {
  ...
  const textContent = notification?.message || notification?.body || "New notification received";
  const dateStr = notification?.createdAt ? new Date(notification.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';
  ...
  const getTypeConfig = (type) => {
    switch (type) {
      ...
      case 'PET_LIVE':
      case 'PET_UPDATED':
        return { icon: "paw", color: Themes.COLORS.primaryDark, bg: Themes.COLORS.soft }; // mint green
      case 'PET_DELETED':
      ...
      default:
        ...
        if (type === 'Approved') return { icon: "checkmark-circle-sharp", color: "#4CAF50", bg: "#E8F5E9" };
        if (type === 'Cancelled') return { icon: "alert-circle-sharp", color: "#FF9800", bg: "#FFF3E0" };
        if (type === 'Rejected') return { icon: "close-circle-sharp", color: "#F44336", bg: "#FFEBEE" };
        if (type === 'Pending') return { icon: "ellipsis-horizontal-circle-sharp", color: "#2196F3", bg: "#E3F2FD" };
        
        return { icon: "notifications", color: Themes.COLORS.textMuted, bg: "#F5F5F5" };
    }
  };
  ...
  const handlePress = async () => {
    ...
    try {
      if (type.startsWith('ADOPTER_')) {
        ...
        const res = await api.get(`/api/adoptionApp/${notification.relatedEntity}`);
        const appData = res.data?.body;
        if (appData?.petToAdopt) {
          navigation.navigate("viewApplicantsMyAdoptees", { petID: appData.petToAdopt });
        }
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.textContainer}>
        <Text style={[styles.bodyText, !isRead && styles.unreadText]} numberOfLines={2}>
          {textContent}
        </Text>
